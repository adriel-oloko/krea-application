...
export default function Header() {
	...
	return (
		<header className="relative md:fixed z-50 -top-0 w-full mx-auto grid grid-cols-2 lg:grid-cols-3 items-center bg-white p-4 text-[#a3a3a3] dark:bg-black/50 dark:backdrop-blur dark:text-[#737373] md:px-8">
			<button
				title="menu open"
			...
			<div className="hidden lg:flex gap-8 items-center text-black dark:text-white">
				...
				<div className="flex gap-2 items-center">
					<Image
						...
						alt=""
					/>
					<span className="font-medium text-[#a3a3a3] w-32 truncate">
					...
					<svg
						...
						width="24px"
						fill="currentColor"
						className="rotate-90 translate-y-0.25">
			...
			<div className="heroHeaderNav bg-primary-100 rounded-2xl p-2 hidden lg:grid grid-cols-7 gap-0 dark:bg-primary-850 w-fit mx-auto text-black dark:text-white relative z-50 font-inter">
				<button
					title="Home"
					type="button"
					onClick={() => setIsHomeMenu((prev) => !prev)}
					className="bg-white py-5 px-6 w-fit relative rounded-xl shadow-[0_1px_4px_0px_rgba(0,0,0,0.075)] dark:shadow-none dark:text-black">
					<svg
						width="18"
						height="18"
						viewBox="0 0 24 24"
				...
				<button
					title="Gallery"
					type="button"
					className="py-5 px-6 w-fit relative rounded-xl dark:shadow-none">
					<ImageIcon size={18} />
				</button>
				<button
					title="Video"
					type="button"
					className="py-5 px-6 w-fit relative rounded-xl dark:shadow-none">
					<VideoIcon size={18} className="fill-current" />
				</button>
				<button
					title="Enhancer"
					type="button"
					className="py-5 px-6 w-fit relative rounded-xl dark:shadow-none">
					<WandSparklesIcon size={18} className="-rotate-90" />
				</button>
				<button
					title="Realtime"
					type="button"
			...
			<AnimatePresence>{isHomeMenu && <HomeHover />}</AnimatePresence>

			<div className="flex gap-2 justify-end items-center text-black dark:text-white">
				<button
					title={"gallery"}
					type="button"
					className="hover:bg-primary-150 focus:ring-1 rounded-xl bg-primary-100 p-2 xl:px-4 py-2 h-fit w-fit flex gap-2 items-center relative dark:bg-primary-850 dark:hover:bg-primary-850">
					<ImageIcon className="size-4.5" />
					<span className="hidden xl:inline-block translate-y-0 font-medium text-sm">
						Gallery
					</span>
				</button>
				...
				<button
					...
					type="button"
					className="hover:bg-primary-150 focus:ring-1 rounded-xl bg-primary-100 p-2 xl:px-4 py-2 h-fit w-fit flex gap-2 items-center relative dark:bg-primary-850 dark:hover:bg-primary-850">
					<HeadsetIcon className="size-4.5" />
					<span className="hidden xl:inline-block translate-y-0 font-medium text-sm">
						Support
					</span>
				</button>
				...
				<button
					title={"notification"}
					type="button"
					className="hover:bg-primary-150 focus:ring-1 rounded-xl bg-primary-100 p-2 h-fit w-fit flex gap-2 items-center relative dark:bg-primary-850 dark:hover:bg-primary-850">
					<BellIcon className="size-4.5 fill-current" />
				</button>

				<button
					onClick={toggleTheme}
					aria-label="Theme"
					className="hover:bg-primary-150 focus:ring-1 rounded-xl bg-primary-100 p-2 h-fit w-fit flex gap-2 items-center relative dark:bg-primary-850 dark:hover:bg-primary-900">
					{theme == "light" ? (
						<MoonIcon className="rotate-270 size-4.5 fill-current" />
					) : (
						<SunIcon className="size-4.5 fill-current" />
					)}
				</button>
				<Image
					src={"/gradient-blob.svg"}
...
export function Navbar({ closeFunc }: NavbarProps) {
	return (
		<motion.section
			animate={{ opacity: 1 }}
			exit={{ opacity: 0 }}
			className="z-30 bg-white dark:bg-primary-950 opacity-0 flex flex-col gap-5 px-4 py-1 pb-4 h-screen overflow-y-auto absolute inset-y-0 left-0 w-[calc(100sv-4rem)] max-w-[360px] transition-transform transition-discrete duration-[650ms] ease-[cubic-bezier(0.22,1,0.36,1)]">
			<button
				title="menu close"
				onClick={closeFunc}
				type="button"
				className="p-3 -translate-x-2.5 text-black dark:text-white">
				<svg
					xmlns="http://www.w3.org/2000/svg"
					width="20"
					height="20"
			...
			<Link
				href={"#"}
				className="text-black dark:text-white flex gap-2 items-center">
			...
			<div className="flex flex-col gap-4 mt-4">
				<h2 className="text-black dark:text-white font-medium text-xl">
					Account
				</h2>
					...
					<Link
						...
						<div className="bg-primary-800 dark:bg-action p-2.5 rounded-xl w-fit text-white flex gap-2.5">
							<svg
								...
								aria-hidden="true"
								role="img"
								className="translate-x-0.5 iconify iconify--lets-icons"
								width="22"
								height="22"
								viewBox="0 0 24 24">
								<g fill="none">
									...
									<path
										...
										strokeLinecap="round"
										strokeWidth="2"
										d="M19 10v6m3-3h-6"></path>
									<path
										fill="currentColor"
										d="M17.142 20.383c.462-.105.739-.585.534-1.012c-.552-1.15-1.459-2.162-2.634-2.924C13.595 15.509 11.823 15 10 15s-3.595.508-5.042 1.447c-1.175.762-2.082 1.773-2.634 2.924c-.205.427.072.907.534 1.012a32.3 32.3 0 0 0 14.284 0"></path>
								</g>
							</svg>
						</div>
						...
						<Link
							...
							<div className="relative bg-primary-150/15 dark:bg-primary-850/15 p-2.5 rounded-xl dark:text-primary-150 min-size-6 min-w-11">
								<svg
									...
									aria-hidden="true"
									role="img"
									className="absolute inset-0 m-auto h-5.5 w-5.5 transition-transform duration-200 ease-out group-hover:scale-110 iconify iconify--akar-icons"
									width="1em"
									height="1em"
									viewBox="0 0 24 24">
									<g
...
export function HomeHover() {
	return (
		<motion.div
			...
			className="homeHoverContainer pt-24 pl-16 pr-12 pb-12 absolute z-20 dark:bg-primary-900 dark:border-primary-850 border-primary-150 w-full overflow-hidden rounded-2xl border-[3px] bg-white shadow-[0_84px_32px_-40px_rgba(0,0,0,0.05)] max-w-5xl mx-auto inset-x-0 top-1.5 dark:shadow-[0_84px_32px_-40px_rgba(0,0,0,0.075)] hidden md:grid grid-cols-3 gap-4">
...
export function HomeHoverButton({ title, image }: HomeHoverButtonProps) {
	return (
		<button
			title={title}
			type="button"
			className="homeHover p-4 focus:ring-1 rounded-xl w-full flex gap-4 items-center relative text-black dark:text-white text-sm hover:bg-black/5 dark:hover:bg-white/5">
			<Image
				...
				alt=""
			/>
			<span className="text-base font-inter">{title}</span>
		</button>
	);
}
